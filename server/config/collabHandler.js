...
module.exports = function(socket, io){
  ...
  if (joinedRoom === null) {
    ...
    socket.on('solutionFound', function(){
      socket.broadcast.to(joinedRoom.id).emit('endCollabSession');
    });


    socket.on('clickedAttempting', function(){
      console.log('clickedAttempting');
      socket.broadcast.to(joinedRoom.id).emit('partnerClickedAttempting');
    });

    socket.on('userReceivedAttemptMessage', function(data){
      socket.broadcast.to(joinedRoom.id).emit('sendPartnerAttemptMessage', data);
    });
